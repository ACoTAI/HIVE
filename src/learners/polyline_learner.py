...
from modules.mixers.PolylineQ import PolylineQ
from utils.rl_utils import build_td_lambda_targets

class PolylineLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer is not None:
            if args.mixer == "PolylineQ":
                self.mixer = PolylineQ(args)
            else:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        mac_out_ = mac_out.clone().detach()
        mac_out_[avail_actions == 0] = -9999999
        cur_max_actions = mac_out_[:, :-1].max(dim=3, keepdim=True)[1]
        is_max_action = (cur_max_actions == actions).float()
        obs_act = th.cat([obs, F.one_hot(actions_.squeeze(-1), num_classes=self.args.n_actions)], dim=-1)
        ...
        if self.mixer is not None:
            chosen_action_qvals = self.mixer(
                batch["state"][:, :-1], chosen_action_qvals, obs_act[:, :-1], is_max_action
            )
            target_max_qvals = self.target_mixer(
                batch["state"][:, :], target_max_qvals, obs_act[:, :], th.ones_like(target_max_qvals.unsqueeze(3))
            )
        ...
        # targets = (
        #     rewards + self.args.gamma * (1 - terminated) * target_max_qvals[:, 1:].detach()
        # )
