...
class VAOSLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if "matrix" in self.args.name and self.need_print:
            ...
            actions_ = th.tensor(actions_).unsqueeze(0).unsqueeze(0).expand(actions.size(0), actions.size(1), -1, -1)
        ...
        if ave:
            ...
            for t in range(batch.max_seq_length):
                ...
                q_target = q_target.to(agent_outs.device)
        ...
        if self.mixer is not None:
            ...
            if t_env % (self.args.trans*25) == 0:
                ...
                mix_q_target = mix_q_target.to(target_max_qvals.device)
        ...
        if "matrix" in self.args.name and self.need_print:
            ...
            for i in range(self.args.n_actions ** self.args.n_agents):
                ...
                tot_q = self.mixer(demo_action_qvals, batch["state"][:, :-1])
