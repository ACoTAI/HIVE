...
from modules.mixers.HPQ import HPQMixer
...
class HPQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer is not None:
            self.mixer = HPQMixer(args)
            self.params += list(self.mixer.parameters())
            self.target_mixer = copy.deepcopy(self.mixer)
        self.feat = copy.deepcopy(self.mac)
        self.params += list(self.feat.parameters())
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        feat_mac_out = []
        self.feat.init_hidden(batch.batch_size)
        for t in range(batch.max_seq_length):
            agent_outs = self.feat.forward(batch, t=t)
            feat_mac_out.append(agent_outs)
        feat_mac_out = th.stack(feat_mac_out, dim=1)  # Concat over time
        feat_action_qvals = th.gather(feat_mac_out[:, :-1], dim=3, index=actions).squeeze(3)
        ...
        if self.mixer is not None:
            chosen_action_qvals = self.mixer(chosen_action_qvals, batch["state"][:, :-1], batch['obs'][:, :-1], max_filter, target=False, feat=feat_action_qvals)
            target_max_qvals = self.target_mixer(target_max_qvals, batch["state"][:, 1:], batch['obs'][:, 1:], None, target=True)
        if self.args.standardise_returns:
            target_max_qvals = (
