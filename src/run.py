...
def run(_run, _config, _log):
    ...
    unique_token = (
        f"{_config['name']}_{map_name}_{datetime.datetime.now()}"
    )
...
def run_sequential(args, logger):
    ...
    if hasattr(args, 'vaos') and args.vaos: scheme['future_discounted_return'] = {"vshape": (1,)}
    ...
    while runner.t_env <= args.t_max:
        ...
        if hasattr(args, 'vaos') and args.vaos:
